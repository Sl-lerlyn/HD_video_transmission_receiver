...
//	}
    ...
    //if ((totalFramesScheduled % framesPerSecond) == 0)  currentFrame = videoFrameBars;

    //begin*****************************************************************
    //int frameSize = videoFrameBlack->GetRowBytes() * videoFrameBlack->GetHeight();
    uint8_t* buffer;
    currentFrame->GetBytes((void**)&buffer);
    getFrame(buffer);
    //end ***********************************************************
    ...
    result = deckLinkOutput->ScheduleVideoFrame(currentFrame.get(), (totalFramesScheduled * 1000)-20000, 1000, 25000);
    if (result != S_OK)
    {
...
//	}
    ...
    for (int i=0; i<25; i++){
        getpcm((uint16_t*)audioBuffer);
        printf("%d %d\n", totalAudioSecondsScheduled, i);
        while (totalAudioSecondsScheduled*25+i > totalFramesScheduled) std::this_thread::sleep_for(std::chrono::milliseconds(10));
        //if (selectedDevice->getDeviceOutput()->ScheduleAudioSamples(audioBuffer, 480, (totalAudioSecondsScheduled * audioBufferSampleLength)+i*480, audioSampleRate, nullptr) != S_OK)
        if (selectedDevice->getDeviceOutput()->ScheduleAudioSamples(audioBuffer, 480*4, (totalAudioSecondsScheduled*25+i)*1000-20000, 25000, nullptr) != S_OK)
            return;
    }
