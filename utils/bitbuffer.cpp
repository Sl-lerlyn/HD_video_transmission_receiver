...
uint8_t LowestBits(uint8_t byte, size_t bit_count) { return byte & ((1 << bit_count) - 1); }
...
uint8_t WritePartialByte(uint8_t source, size_t source_bit_count, uint8_t target,
...
bool BitBuffer::ReadUInt32(uint32_t* val) { return ReadBits(val, sizeof(uint32_t) * 8); }
...
bool BitBuffer::ConsumeBytes(size_t byte_count) { return ConsumeBits(byte_count * 8); }
...
void BitBuffer::GetCurrentOffset(size_t* out_byte_offset, size_t* out_bit_offset) {
